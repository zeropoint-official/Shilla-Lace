    ...
    <>
      ...
      >
        <div className="max-w-[1400px] mx-auto px-5 md:px-10">
          <div className="grid grid-cols-[auto_1fr_auto] lg:grid-cols-[1fr_auto_1fr] items-center h-16 md:h-20 gap-2">
              ...
              <nav className="hidden lg:flex items-center gap-7">
                {navigation.map((item) => (
                  <Link
                    key={item.href}
                    href={item.href}
                    className="text-[10px] tracking-[0.2em] uppercase text-cream/60 hover:text-cream transition-colors duration-300 whitespace-nowrap"
                  >
                    {item.title}
                  </Link>
                ))}
